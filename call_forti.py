...
def ensure_service_exists(port, protocol, headers):
    ...
    resp = requests.get(url, headers, VERIFY_SSL)
    if resp.status_code == 404:
        #service chua ton tai, tao moi
        data = {
            "name" : service_name,
            "protocol": "TCP/UDP",
        }
        if protocol.lower() == "tcp":
            ...
            raise ValueError(f"Protocol khong hop le: {protocol}")
        create_resp = requests.post (
        ...
        if not create_resp.ok:
            raise Exception(f"khong the tao service {service_name}: {create_resp.text}")
    elif not resp.ok:
        raise Exception(f"loi khi kiem tra service: {service_name}: {resp.text}")
    elif resp.status_code == 200:
        print(f"service da ton tai: {service_name}")
        return service_name
    return service_name
...
def get_issue_details(issue_key):
    ...
    # print( f"{JIRA_BASE_URL}/rest/proformalite/api/2/portal/1/issues/{issue_key}")
    # print(f"Bearer {JIRA_API_TOKEN}")
    print(issue_key)
...
def create_firewall_rule():
    data = request.get_json()
    issue = data[0]  # Giả sử là danh sách

   # Trích xuất thông tin
    payload_fw = {
                "source_ip" : issue["state"]["answers"]["13"]["text"],
                "dest_ip" : issue["state"]["answers"]["14"]["text"],
                "port_udp" : issue.get("state").get("answers").get("15", {}).get("text"),
                "port_udp" : issue.get("state").get("answers").get("18", {}).get("text"),
                #"purpose" : issue["state"]["answers"]["12"]["text"]
            }
